...
function summarizeRound(round) {
  ...
  const score = sum(holes, "score");
  const adjustedScore = round.adjustedScore ?? null;
  const par = sum(holes, "par");
  const putts = sum(holes, "putts");
  const threePutts = holes.filter((hole) => hole.putts >= 3).length;
  const gir = holes.filter((hole) => hole.gir).length;
  const fairwayAttempts = holes.filter((hole) => hole.fairway !== null).length;
  const fairways = holes.filter((hole) => hole.fairway === "hit").length;
  ...
  return {
    ...
    par,
    overPar: score - par,
    adjustedOverPar: adjustedScore === null ? null : adjustedScore - par,
    putts,
    threePutts,
    gir,
    fairways,
    ...
    girMisses,
    fairwayMisses,
    girPercent: percent(gir, holes.length),
    fairwayPercent: percent(fairways, fairwayAttempts),
    scoring,
  };
}
...
function summarizeAll(sourceRounds, filter = "all") {
  ...
  const fairwayAttempts = summaries.reduce((total, round) => total + round.fairwayAttempts, 0);
  const fairways = summaries.reduce((total, round) => total + round.fairways, 0);
  const girMisses = countBy(holes.filter((hole) => !hole.gir && isMissDirection(hole.girMiss)).map((hole) => hole.girMiss));
  const fairwayMisses = countBy(
    holes.filter((hole) => isMissDirection(hole.fairway)).map((hole) => hole.fairway),
  );
  ...
  return {
    ...
    averageBasis,
    scoringAverage,
    puttsPerRound: summaries.reduce((total, round) => total + basisValue(round.putts, round, averageBasis), 0) / roundsPlayed,
    threePutts: summaries.reduce((total, round) => total + round.threePutts, 0),
    threePuttsPerRound: summaries.reduce((total, round) => total + basisValue(round.threePutts, round, averageBasis), 0) / roundsPlayed,
    threePuttPercent: percent(holes.filter((hole) => Number(hole.putts || 0) >= 3).length, holes.length),
    girPercent: percent(holes.filter((hole) => hole.gir).length, holes.length),
    fairwayPercent: percent(fairways, fairwayAttempts),
    girMisses,
    fairwayMisses,
...
function renderKpis(stats) {
  ...
  const kpis = [
    ...
    ["Putts / Round", formatNumber(stats.puttsPerRound, 1), basisLabel],
    ["3-Putts / Round", formatNumber(stats.threePuttsPerRound, 1), basisLabel],
    ["3-Putt Rate", `${formatNumber(stats.threePuttPercent, 0)}%`, "Holes with 3+ putts"],
    ["GIR", `${formatNumber(stats.girPercent, 0)}%`, "Greens in regulation"],
    ["Fairways", `${formatNumber(stats.fairwayPercent, 0)}%`, "Par 3s excluded"],
...
function renderHoleStats(sourceRounds) {
  ...
  renderCoursePersonalRecords(sourceRounds, selectedCourse);
...
function renderCourseRecordCard(courseKey, rounds) {
  ...
  const bestRound = minBy(sorted, (round) => round.score);
  const worstRound = maxBy(sorted, (round) => round.score);
  const bestToPar = minBy(sorted, (round) => round.overPar);
  const worstToPar = maxBy(sorted, (round) => round.overPar);
  const bestPutting = minBy(sorted, (round) => round.putts);
  const worstPutting = maxBy(sorted, (round) => round.putts);
  const bestGir = maxBy(sorted, (round) => round.girPercent);
  const worstGir = minBy(sorted, (round) => round.girPercent);
  const bestFairway = maxBy(sorted.filter((round) => round.fairwayAttempts > 0), (round) => round.fairwayPercent);
  const worstFairway = minBy(sorted.filter((round) => round.fairwayAttempts > 0), (round) => round.fairwayPercent);
  ...
  return `
    <article class="record-card">
      <div class="record-card-header">
        ...
        <small>${formatNumber(rounds.reduce((sum, round) => sum + round.score, 0) / rounds.length, 1)} avg</small>
      </div>
      <div class="record-columns">
        <div class="record-column best">
          <h5>Personal Bests</h5>
          ${renderRecordLine("Best Score", bestRound, `${bestRound.score} (${signed(bestRound.overPar, 0)})`)}
          ${renderRecordLine("Best To Par", bestToPar, `${signed(bestToPar.overPar, 0)}`)}
          ${renderRecordLine("Fewest Putts", bestPutting, `${bestPutting.putts}`)}
          ${renderRecordLine("Best GIR", bestGir, `${formatNumber(bestGir.girPercent, 0)}%`)}
          ${bestFairway ? renderRecordLine("Best Fairways", bestFairway, `${formatNumber(bestFairway.fairwayPercent, 0)}%`) : ""}
        </div>
        <div class="record-column worst">
          <h5>Personal Worsts</h5>
          ${renderRecordLine("Worst Score", worstRound, `${worstRound.score} (${signed(worstRound.overPar, 0)})`)}
          ${renderRecordLine("Worst To Par", worstToPar, `${signed(worstToPar.overPar, 0)}`)}
          ${renderRecordLine("Most Putts", worstPutting, `${worstPutting.putts}`)}
          ${renderRecordLine("Worst GIR", worstGir, `${formatNumber(worstGir.girPercent, 0)}%`)}
          ${worstFairway ? renderRecordLine("Worst Fairways", worstFairway, `${formatNumber(worstFairway.fairwayPercent, 0)}%`) : ""}
        </div>
      </div>
    </article>
  `;
