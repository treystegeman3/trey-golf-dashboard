...
function renderSeasonPage(sourceRounds) {
  ...
  const archive = document.querySelector("#season-archive-grid");
  if (archive) renderSeasonArchive(archive, sourceRounds);
...
function renderSeasonArchive(container, sourceRounds) {
  const years = getAvailableYears(sourceRounds);
  if (!years.length) {
    container.innerHTML = `<article class="season-archive-card"><span>No seasons yet</span><strong>—</strong><small>Add rounds to start building the archive.</small></article>`;
    return;
  }

  container.innerHTML = years.map((year) => {
    const yearRounds = filterRoundsByYear(sourceRounds, year).map(summarizeRound);
    const rounds18 = yearRounds.filter((round) => round.holesPlayed === 18);
    const rounds9 = yearRounds.filter((round) => round.holesPlayed === 9);
    const fullRounds = yearRounds.filter((round) => round.hasAdvancedStats);
    const best18 = rounds18.length ? minBy(rounds18, (round) => Number(round.score)) : null;
    const best9 = rounds9.length ? minBy(rounds9, (round) => Number(round.score)) : null;
    const avg18 = rounds18.length ? formatNumber(avg(rounds18, "score"), 1) : "—";
    const avg9 = rounds9.length ? formatNumber(avg(rounds9, "score"), 1) : "—";
    const label = year === "2026" ? "Current loaded season" : "Archived season";
    return `
      <article class="season-archive-card ${year === "2026" ? "active-season" : ""}">
        <span>${label}</span>
        <strong>${year}</strong>
        <small>${yearRounds.length} rounds • ${rounds18.length} 18-hole • ${rounds9.length} 9-hole</small>
        <div class="season-archive-metrics">
          <b>Best 18: ${best18 ? best18.score : "—"}</b>
          <b>Best 9: ${best9 ? best9.score : "—"}</b>
          <b>Avg 18: ${avg18}</b>
          <b>Avg 9: ${avg9}</b>
        </div>
        <em>${fullRounds.length} full-stat rounds • ${yearRounds.length - fullRounds.length} score-only</em>
      </article>
    `;
  }).join("");
}
